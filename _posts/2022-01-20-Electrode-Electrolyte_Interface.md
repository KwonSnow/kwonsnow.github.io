...
하지만 PB 모델도 정확한 theory를 바탕으로 하지는 않는다. 즉, non-locality and non-linearity of dieletric response of water 그리고 surface tension associated with formation of the liquid-solid interface를 고려하지 않는다. 예를들어, (modified) PB 모델은 주로 linear local dielectric response를 이용하지만, 실제로 3 Å double layer 두께정도면 0.1 V drop이 일어나는데, (즉 field가 300 MV/m 정도) 이것은 bulk water의 dieletric constant의 1/3정도에 해당한다. 따라서 liquid-solid interface에서는 non-linear dielectric saturation effect가 존재한다.  
